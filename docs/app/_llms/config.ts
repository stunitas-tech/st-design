...
const GITHUB_OWNER = "stunitas-tech";
const GITHUB_REPO = "st-design";
...
export const sectionConfigs: Record<Section, SectionConfig> = {
    ...
    docs: {
        ...
        baseUrl: "/docs",
        description:
            "컴포넌트 디자인 가이드라인, Foundation (색상, 타이포그래피, 간격 등)",
...
export function getGitHubSourceUrl(section: Section, pagePath: string): string {
    ...
    const encodedPagePath = pagePath
        .split("/")
...
export function shouldIncludeInFullText(
    section: Section,
    pagePath: string,
): boolean {
    const config = sectionConfigs[section];
